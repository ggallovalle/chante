...
type FailWith = (
  ...issues: KdlIssue[]
) => Effect.Effect<never, Schema.SchemaError, never>

const getLocationStrict = (
...
export const parseFromCli = Effect.fnUntraced(function* (
  ...
  if (Option.isNone(flag)) {
    ...
    if (!(yield* fs.exists(defaultConfig))) {
      return yield* new CliError.UserError({
        cause: `Inferred config file not found: ${defaultConfig}`,
      })
    }
...
const getSettings = Effect.fn("getSettings")(function* (
  root: Document,
  failWith: FailWith,
) {
  ...
  const settingsLocation = Predicate.isUndefined(settingsNode)
    ? rootLocation
    : getLocationStrict(settingsNode)
  ...
  const resolve = (
    ...
    fallback: Effect.Effect<
      string,
      Schema.SchemaError | Config.ConfigError,
      never
    >,
  ) =>
    Effect.gen(function* () {
      const value = !Predicate.isUndefined(node)
        ? yield* argumentString(node, 0, failWith)
        : yield* fallback
      const location = !Predicate.isUndefined(node)
        ? getLocationStrict(node)
        : settingsLocation
  ...
  const config = yield* resolve(
    "config",
    configNode,
    Effect.orElseSucceed(env.XDG_CONFIG_HOME.asEffect(), () =>
      pathM.join(home.value, ".config"),
    ),
  )
  const cache = yield* resolve(
    "cache",
    cacheNode,
    Effect.orElseSucceed(env.XDG_CACHE_HOME.asEffect(), () =>
      pathM.join(home.value, ".cache"),
    ),
  )
  const data = yield* resolve(
    "data",
    dataNode,
    Effect.orElseSucceed(env.XDG_DATA_HOME.asEffect(), () =>
      pathM.join(home.value, ".local", "share"),
    ),
  )
  const bin = yield* resolve(
    "bin",
    binNode,
    Effect.orElseSucceed(env.XDG_BIN_HOME.asEffect(), () =>
      pathM.join(home.value, ".local", "bin"),
    ),
  )
  const dotfiles = yield* resolve(
    "dotfiles",
    dotfilesNode,
    Effect.orElseSucceed(env.DOTFILES.asEffect(), () =>
      pathM.join(home.value, "dotfiles"),
    ),
  )
  ...
  const missingPaths = (yield* Effect.all(pathChecks, {
    concurrency: "unbounded",
  })).filter((issue): issue is KdlIssue => issue !== undefined)
...
const parseFileOps = Effect.fn("parseFileOps")(function* (
...
export const parseFrom = Effect.fn("parseFrom")(function* (opts: ParseContext) {
  ...
  for (const pkg of doc
    .findNodeByName("packages")
    ?.findNodesByName("package") ?? []) {
  ...
  for (const bundle of doc
    .findNodeByName("bundles")
    ?.findNodesByName("bundle") ?? []) {
  ...
  return yield* ChanteConfig.decodeUnknown({
    settings,
    packages: configPkgs,
